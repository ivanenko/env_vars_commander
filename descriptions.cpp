...
pair descr[] = {
        ...
        {"PATH", "When you type a command to run, the system looks for it in the directories specified by PATH in the order specified"},
        {"CDPATH", "The search path for the cd command"},
        {"MANPATH", "List of directories for the system to search manual pages in"},
        {"LD_LIBRARY_PATH", "List of directories where the system searches for runtime libraries in addition to those hard-defined by ld and in /etc/ld.so.conf.d/*.conf files"},
        ...
        {"EDITOR", "The name of the user's preferred text editor. Used by programs such as the mutt mail client and sudoedit"},
        {"VISUAL", "Similar to the \"EDITOR\" environment variable, applications typically try the value in this variable first before falling back to \"EDITOR\" if it isn't set"},
        {"BROWSER", "The name of the user's preferred web browser"},

        // Graphical desktop-related variables
        {"DISPLAY", "This variable is used to indicate to graphical applications where to display the actual graphical user interface"},
        {"XDG_DATA_HOME", "Indicates to applications that conform to the freedesktop.org specifications, where to place the user's private data. This variable is typically unset since a sensible default fall-back value was defined by the specifications"},
        {"XDG_CONFIG_HOME", "Where to place the user's configuration information"},
        {"XDG_DATA_DIRS", "A colon-separated list (similar to \"PATH\") of directories where data is searched for"},
        {"XDG_CONFIG_DIRS", "A colon-separated list (similar to \"PATH\") of directories where configuration information is searched for"},
        {"XDG_CACHE_HOME", "A location used by applications that conform to the freedesktop.org specifications to cache temporary data"},

        // Program execution variables
        {"LD_PRELOAD", "This variable can be used to inject a custom dynamic library into an application's memory when it loads"},
        ...
        {"LIBRARY_PATH", "A list of directories (separated by colons) in which library files should be searched for"},
        {"INCLUDE", "A colon-separated list of directories in which header files should be searched for"},
        {"CPATH", "A colon-separated list of directories in which header files should be searched for"},
        ...
        {"COLUMNS", "The number of text columns in the terminal window. Try adjusting this variable if lines don't seem to wrap properly in the console"},
        {"LINES", "The number of text lines on the console window. Try adjusting this variable if you're getting strange results when scrolling text"},
